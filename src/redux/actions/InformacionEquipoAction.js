...
export const onGetColeccion = (
  ...
  const nombreAux = nombre_equipo ? nombre_equipo : '';
  const tipo_equipoAux = tipo_equipo ? tipo_equipo : '';
    ...
    jwtAxios
      .get('informacion-equipos', {
        params: {
          page: page,
          limite: rowsPerPage,
          nombre_equipo: nombreAux,
          tipo_equipo: tipo_equipoAux,
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    jwtAxios
      .put('informacion-equipos/' + params.id, params)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
            type: UPDATE_INFORMACION_EQUIPO,
            payload: data.data,
          });
          updateColeccion();
          handleOnClose();
...
export const onDelete = (id) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    jwtAxios
      .delete('informacion-equipos/' + id)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({type: DELETE_INFORMACION_EQUIPO, payload: data.data});
        } else {
...
export const onCreate = (params, handleOnClose, updateColeccion) => {
  ...
  return (dispatch) => {
    dispatch({type: FETCH_START});
    jwtAxios
      .post('informacion-equipos', params)
      .then((data) => {
        console.log(data);
        if (data.status === 201) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
            type: CREATE_INFORMACION_EQUIPO,
            payload: data.data,
          });
          updateColeccion();
          handleOnClose();
