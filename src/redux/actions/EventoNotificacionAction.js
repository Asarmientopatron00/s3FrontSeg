...
export const onGetColeccion = (
  ...
  return (dispatch) => {
    ...
    jwtAxios
      .get('http://solicitudesservicio.test/api/eventos-notificacion', {
        params: {
          page: page,
          limite: rowsPerPage,
          nombre: nombreAux,
...
export const onShow = (id) => {
  ...
  return (dispatch) => {
    if (id !== 0) {
      dispatch({type: FETCH_START});
      jwtAxios
        .get('http://solicitudesservicio.test/api/eventos-notificacion/' + id)
        .then((data) => {
          if (data.status === 200) {
            dispatch({type: FETCH_SUCCESS});
...
export const onDelete = (id) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    jwtAxios
      .delete('http://solicitudesservicio.test/api/eventos-notificacion/' + id)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
...
export const onCreate = (params, handleOnClose, updateColeccion) => {
  ...
  return (dispatch) => {
    dispatch({type: FETCH_START});
    jwtAxios
      .post('http://solicitudesservicio.test/api/eventos-notificacion', params)
      .then((data) => {
        if (data.status === 201) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
