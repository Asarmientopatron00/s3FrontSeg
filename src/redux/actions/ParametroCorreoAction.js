import {
  GET_COLECCION_PARAMETRO_CORREO,
...
export const onGetColeccion = (
  ...
  return (dispatch) => {
    ...
    Api.get('http://solicitudesservicio.test/api/parametros-correos', {
      params: {
        page: page,
        limite: rowsPerPage,
        nombre: nombreAux,
    ...
    Api.get('http://solicitudesservicio.test/api/parametros-correos', {
      ...
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
            type: GET_COLECCION_LIGERA_PARAMETRO_CORREO,
            payload: data,
          });
        } else {
          dispatch({
...
export const onShow = (id) => {
  ...
  return (dispatch) => {
    if (id !== 0) {
      dispatch({type: FETCH_START});
      Api.get('http://solicitudesservicio.test/api/parametros-correos/' + id)
        .then((data) => {
          if (data.status === 200) {
            dispatch({type: FETCH_SUCCESS});
...
export const onUpdate = (params, handleOnClose, updateColeccion) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    Api.put(
      'http://solicitudesservicio.test/api/parametros-correos/' + params.id,
      params,
    )
      .then((data) => {
        if (data.status === 200) {
...
export const onDelete = (id) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    Api.delete('http://solicitudesservicio.test/api/parametros-correos/' + id)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
...
export const onCreate = (params, handleOnClose, updateColeccion) => {
  ...
  return (dispatch) => {
    dispatch({type: FETCH_START});
    Api.post('http://solicitudesservicio.test/api/parametros-correos', params)
      .then((data) => {
        if (data.status === 201) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
            type: CREATE_PARAMETRO_CORREO,
            payload: data.data,
          });
          updateColeccion();
          handleOnClose();
