...
export const onGetColeccion = (currentPage,rowsPerPage,nombre,orderByToSend) => {
  ...
  return (dispatch) => {
    ...
    Api.get('http://10.1.1.14/solicitudesservicio-backend/public/api/roles', {
      params: {
        page: page,
        limite:rowsPerPage,
        nombre:nombreAux,
...
export const onShow = (id) => {
  ...
  return (dispatch) => {
    if(id!==0){
      dispatch({type: FETCH_START});
      Api.get('http://10.1.1.14/solicitudesservicio-backend/public/api/roles/'+id)
        .then((data) => {
          if (data.status === 200) {
            dispatch({type: FETCH_SUCCESS});
...
export const onUpdate = (params) => {
  return (dispatch) =>  {
    dispatch({type: FETCH_START});
    Api.put('http://10.1.1.14/solicitudesservicio-backend/public/api/roles/' + params.id, params)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
          dispatch({
...
export const onDelete = (id) => {
  return (dispatch) => {
    dispatch({type: FETCH_START});
    Api.delete('http://10.1.1.14/solicitudesservicio-backend/public/api/roles/'+id)
      .then((data) => {
        if (data.status === 200) {
          dispatch({type: FETCH_SUCCESS});
...
export const onCreate = (params) => {
  ...
  return (dispatch) => {
    dispatch({type: FETCH_START});
    Api.post('http://10.1.1.14/solicitudesservicio-backend/public/api/roles',params)
      .then((data) => {
        console.log(data);
        if (data.status === 201) {
          dispatch({type: FETCH_SUCCESS});
