...
const mensajeValidacion = (type,cant=0)=>{
  switch (type){
    case 'max':
      return (saMessages.messages['validacion.max1']+cant+saMessages.messages['validacion.max2'])
    case 'min':
...
const validationSchema = yup.object({
  ...
  segundo_nombre: yup
    ...
    .max(128,mensajeValidacion('max',128)),
  primer_apellido:yup
    .string().max(128,mensajeValidacion('max',128))
    .nullable()
    .when("tipo_persona", {
      is: 'N',
...
const TerceroServicioCreator = (props) => {
  ...
  const tiposDocumentos = useSelector(({tipoDocumentoReducer}) => tipoDocumentoReducer.ligera);
  ...
  useEffect(() => {
    dispatch(
      tipoDocumentoColeccionLigera(),
    );
    dispatch(
      departamentosColeccionLigera(),
    );
  }, [dispatch]);

  const onChangeDepartamento = (id)=>{
    dispatch(
      ciudadColeccionLigera(id),
    );
  }
  ...
  selectedRow = useSelector(({terceroServicioReducer}) => terceroServicioReducer.selectedRow);
      ...
      <Scrollbar>
        <Formik
          ...
          initialValues={{
            ...
            tipo_documento_id: selectedRow ? selectedRow.tipo_documento_id : '',
            numero_documento: selectedRow ? selectedRow.numero_documento : '',
            digito_verificacion:selectedRow ? selectedRow.digito_verificacion : 0,
