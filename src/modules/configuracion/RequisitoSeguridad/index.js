...
import RequisitoSeguridadCreador from './RequisitoSeguridadCreador'
import {
  onGetColeccion,
  onDelete,
} from '../../../redux/actions/RequisitoSeguridadAction';
      ...
      {numSelected > 0 ? (
        ...
        <Box className={classes.rootBottoms}>
          ...
          <Box className={classes.verticalBottoms}>
            <Tooltip title="Crear Requisito de Seguridad" onClick={onOpenAddRequisitoSeguridad}>
...
const RequisitoSeguridad =  () => {
  ...
  const [requisitoSeguridadSeleccionado,setRequisitoSeguridadSeleccionado]= useState(0);
  const {rows,
          desde,
          hasta,
          ultima_pagina,
          total,
        } = useSelector(({requisitoSeguridadReducer}) => requisitoSeguridadReducer);
  ...
  const onOpenEditRequisitoSeguridad = (id) => {
    setRequisitoSeguridadSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewRequisitoSeguridad = (id) => {
    setRequisitoSeguridadSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteRequisitoSeguridad = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: "¿Seguro Que Desea Eliminar El Requisito de Seguridad?",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Eliminado',
          'El Requisito de Seguridad Fue Eliminado Correctamente',
          'success'
        )
        setTimeout(()=>{
          updateColeccion();
  ...
  const onOpenAddRequisitoSeguridad = () => {
    setRequisitoSeguridadSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
                    ...
                    > 
                      ...
                      <TableCell align="center" className={classes.acciones}> 
                        <Tooltip title={<IntlMessages id='boton.editar'/>}>
                          <EditIcon onClick={()=>onOpenEditRequisitoSeguridad(row.id)} className={`${classes.generalIcons} ${classes.editIcon}`}></EditIcon>
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.ver'/>}>
                          <VisibilityIcon onClick={()=>onOpenViewRequisitoSeguridad(row.id)} className={`${classes.generalIcons} ${classes.visivilityIcon}`}></VisibilityIcon> 
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.eliminar'/>}>
                          <DeleteIcon  onClick={()=>onDeleteRequisitoSeguridad(row.id)} className={`${classes.generalIcons} ${classes.deleteIcon}`}></DeleteIcon> 
                        </Tooltip>
                      </TableCell>
      ...
      /> */}
      {showForm ?
        <RequisitoSeguridadCreador 
          showForm={showForm}
          requisitoSeguridad={requisitoSeguridadSeleccionado}
          accion={accion}
...
export default RequisitoSeguridad;