...
import ActividadEconomicaCreador from './ActividadEconomicaCreador'
import {
  onGetColeccion,
  onDelete,
...
const cells = [
  ...
  {id:'codigo_ciiu', typeHead: 'numeric',label: 'Código CIIU',value:(value)=>value, align:'right',mostrarInicio:true},
      ...
      {numSelected > 0 ? (
        ...
        <Box className={classes.rootBottoms}>
          ...
          <Box className={classes.verticalBottoms}>
            <Tooltip title="Crear Actividad Económica" onClick={onOpenAddActividadEconomica}>
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddActividadEconomica: PropTypes.func.isRequired,
...
const ActividadEconomica =  () => {
  ...
  const [actividadEconomicaSeleccionado,setActividadEconomicaSeleccionado]= useState(0);
  const {rows,
          desde,
          hasta,
          ultima_pagina,
          total,
        } = useSelector(({actividadEconomicaReducer}) => actividadEconomicaReducer);
  ...
  const onOpenEditActividadEconomica = (id) => {
    setActividadEconomicaSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewActividadEconomica = (id) => {
    setActividadEconomicaSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteActividadEconomica = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: "¿Seguro Que Desea Eliminar La Actividad Economica?",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Eliminado',
          'La Actividad Economica Fue Eliminado Correctamente',
          'success'
        )
        setTimeout(()=>{
          updateColeccion();
  ...
  const onOpenAddActividadEconomica = () => {
    setActividadEconomicaSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
                    ...
                    > 
                      ...
                      <TableCell align="center" className={classes.acciones}> 
                        <Tooltip title={<IntlMessages id='boton.editar'/>}>
                          <EditIcon onClick={()=>onOpenEditActividadEconomica(row.id)} className={`${classes.generalIcons} ${classes.editIcon}`}></EditIcon>
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.ver'/>}>
                          <VisibilityIcon onClick={()=>onOpenViewActividadEconomica(row.id)} className={`${classes.generalIcons} ${classes.visivilityIcon}`}></VisibilityIcon> 
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.eliminar'/>}>
                          <DeleteIcon  onClick={()=>onDeleteActividadEconomica(row.id)} className={`${classes.generalIcons} ${classes.deleteIcon}`}></DeleteIcon> 
                        </Tooltip>
                      </TableCell>
      ...
      /> */}
      {showForm ?
        <ActividadEconomicaCreador 
          showForm={showForm}
          actividadEconomica={actividadEconomicaSeleccionado}
          accion={accion}
