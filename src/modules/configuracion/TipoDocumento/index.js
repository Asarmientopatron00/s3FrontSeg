...
const cells = [
  ...
  {id:'codigo', typeHead: 'string',label: 'Código Externo',value:(value)=>value, align:'left',mostrarInicio:true},
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddTipoDocumento: PropTypes.func.isRequired,
...
const TipoDocumento =  () => {
  ...
  const {rows,
          ...
          ultima_pagina,
          total,
        } = useSelector(({tipoDocumentoReducer}) => tipoDocumentoReducer);
  ...
  const onOpenEditTipoDocumento = (id) => {
    setTipoDocumentoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewTipoDocumento = (id) => {
    setTipoDocumentoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteTipoDocumento = (id) => {
    Swal.fire({
      title: 'Confirmar',
  ...
  const onOpenAddTipoDocumento = () => {
    setTipoDocumentoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        <EnhancedTableToolbar 
          ...
          onOpenAddTipoDocumento={onOpenAddTipoDocumento}
