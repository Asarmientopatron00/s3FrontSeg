...
import EventoNotificacionCreador from './EventoNotificacionCreador'
import {
  onGetColeccion,
  onDelete,
      ...
      {numSelected > 0 ? (
        ...
        <Box className={classes.rootBottoms}>
          ...
          <Box className={classes.verticalBottoms}>
            <Tooltip title="Crear Tercero de Servicio" onClick={onOpenAddEventoNotificacion}>
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddEventoNotificacion: PropTypes.func.isRequired,
...
const EventoNotificacion =  () => {
  ...
  const [eventoNotificacionSeleccionado,setEventoNotificacionSeleccionado]= useState(0);
  const {rows,
          desde,
          hasta,
          ultima_pagina,
          total,
        } = useSelector(({eventoNotificacionReducer}) => eventoNotificacionReducer);
  ...
  const onOpenEditEventoNotificacion = (id) => {
    setEventoNotificacionSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewEventoNotificacion = (id) => {
    setEventoNotificacionSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteEventoNotificacion = (id) => {
    Swal.fire({
      title: 'Confirmar',
  ...
  const onOpenAddEventoNotificacion = () => {
    setEventoNotificacionSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        <EnhancedTableToolbar 
          ...
          onOpenAddEventoNotificacion={onOpenAddEventoNotificacion}
                    ...
                    > 
                      ...
                      <TableCell align="center" className={classes.acciones}> 
                        <Tooltip title={<IntlMessages id='boton.editar'/>}>
                          <EditIcon onClick={()=>onOpenEditEventoNotificacion(row.id)} className={`${classes.generalIcons} ${classes.editIcon}`}></EditIcon>
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.ver'/>}>
                          <VisibilityIcon onClick={()=>onOpenViewEventoNotificacion(row.id)} className={`${classes.generalIcons} ${classes.visivilityIcon}`}></VisibilityIcon> 
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.eliminar'/>}>
                          <DeleteIcon  onClick={()=>onDeleteEventoNotificacion(row.id)} className={`${classes.generalIcons} ${classes.deleteIcon}`}></DeleteIcon> 
                        </Tooltip>
                      </TableCell>
      ...
      /> */}
      {showForm ?
        <EventoNotificacionCreador 
          showForm={showForm}
          eventoNotificacion={eventoNotificacionSeleccionado}
          accion={accion}
...
export default EventoNotificacion;