...
import PaisCreador from './PaisCreador'
import {
  onGetColeccion,
  onDelete,
...
const cells = [
  ...
  {id:'codigo_externo', typeHead: 'string',label: 'Código Externo',value:(value)=>value, align:'left',mostrarInicio:true},
      ...
      {numSelected > 0 ? (
        ...
        <Box className={classes.rootBottoms}>
          ...
          <Box className={classes.verticalBottoms}>
            <Tooltip title="Crear Pais" onClick={onOpenAddPais}>
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddPais: PropTypes.func.isRequired,
...
const Pais =  () => {
  ...
  const {rows,
          ...
          ultima_pagina,
          total,
        } = useSelector(({paisReducer}) => paisReducer);
  ...
  const onOpenViewPais = (id) => {
    setPaisSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeletePais = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: "¿Seguro Que Desea Eliminar El Pais?",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Eliminado',
          'El Pais Fue Eliminado Correctamente',
          'success'
        )
        setTimeout(()=>{
          updateColeccion();
                    ...
                    > 
                      ...
                      <TableCell align="center" className={classes.acciones}> 
                        <Tooltip title={<IntlMessages id='boton.editar'/>}>
                          <EditIcon onClick={()=>onOpenEditPais(row.id)} className={`${classes.generalIcons} ${classes.editIcon}`}></EditIcon>
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.ver'/>}>
                          <VisibilityIcon onClick={()=>onOpenViewPais(row.id)} className={`${classes.generalIcons} ${classes.visivilityIcon}`}></VisibilityIcon> 
                        </Tooltip>
                        <Tooltip title={<IntlMessages id='boton.eliminar'/>}>
                          <DeleteIcon  onClick={()=>onDeletePais(row.id)} className={`${classes.generalIcons} ${classes.deleteIcon}`}></DeleteIcon> 
                        </Tooltip>
                      </TableCell>
