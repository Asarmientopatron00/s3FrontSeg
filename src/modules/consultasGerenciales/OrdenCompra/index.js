...
import {
  ...
  onGetPromedios,
  onGetColeccionDatos,
} from '../../../redux/actions/CGOrdenCompraAction';
...
const cells = [
  ...
  {
    id: 'total_dias_despacho',
    typeHead: 'numeric',
    label: 'Total Dias Despacho',
    value: (value) => value,
    align: 'right',
    mostrarInicio: true,
  },
  {
    id: 'total_dias_facturacion',
    typeHead: 'numeric',
    label: 'Total Dias Facturacion',
    value: (value) => value,
    align: 'right',
    mostrarInicio: true,
  },
...
const OrdenCompraConsulta = (props) => {
  ...
  const datosTabla = useSelector(
    ({cGOrdenCompraReducer}) => cGOrdenCompraReducer.ligera,
  );
  ...
  useEffect(() => {
    if (fechaInicialFiltro && fechaFinalFiltro) {
      ...
      dispatch(onGetPromedios(fechaInicialFiltro, fechaFinalFiltro));
      setShowData(true);
    } else {
      setShowData(false);
    }
  ...
  const onOpenViewOrdenCompra = (id) => {
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        ...
        <Box className={classes.marcoTabla}>
          ...
          {showData && (
            <MyTable
              headers={['Estado', 'Numero', 'Porcentaje', 'Dias Promedio']}
              data={datosTabla}
                        ...
                        <TableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewOrdenCompra(row.id)}
...
export default OrdenCompraConsulta;
